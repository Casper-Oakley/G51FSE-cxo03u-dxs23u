...
#process options for highlighted input
					...
					if event.type == pygame.MOUSEBUTTONDOWN:
						...
						if self.menu.playButton.mousePress(pygame.mouse.get_pressed()):
							...
							pygame.quit()
			#send simple input and receive output from options, changing volume
			elif self.inOptions:
				for event in pygame.event.get():
					if event.type == pygame.QUIT:
						pygame.exit()
					...
					if event.type == pygame.MOUSEBUTTONDOWN:
						if self.optionsMenu.volumeUpButton.mousePress(pygame.mouse.get_pressed()):
							...
							self.loadMenu()
			#send simple input and receive output from the high-scores menu
			elif self.inHighscores:
				for event in pygame.event.get():
					if event.type == pygame.QUIT:
						pygame.exit()
					...
					if event.type == pygame.MOUSEBUTTONDOWN:
						if self.highscoresMenu.backButton.mousePress(pygame.mouse.get_pressed()):
							self.loadMenu()
			#send simple input and receive output from the game-over screen
			elif self.inGameover:
				for event in pygame.event.get():
					if event.type == pygame.QUIT:
						pygame.exit()
					...
					if event.type == pygame.MOUSEBUTTONDOWN:
						if self.gameoverMenu.playButton.mousePress(pygame.mouse.get_pressed()):
							...
							self.loadHighscores()
			#send simple input and receive output from the pause menu
			elif self.inPause:
				for event in pygame.event.get():
					if event.type == pygame.QUIT:
						pygame.exit()
					...
					if event.type == pygame.MOUSEBUTTONDOWN:
						if self.pauseMenu.restartButton.mousePress(pygame.mouse.get_pressed()):
							...
							self.inGame = True
							self.inPause = False
			#reset the button cool down counter
			self.counter = 0
